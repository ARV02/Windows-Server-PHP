...
if($conectado){
    echo "Conectado";
    echo "<br>";
    $autenticado = @ldap_bind($conectado, "Administrador@tijuana.empresa.com", "Admin1-");
    if($autenticado){
        $se=ldap_search($conectado, "dc=tijuana,dc=empresa, dc=com", "(CN=*)");
        ...
        for ($i=0; $i <$infos["count"]; $i++) { 
        	if ($infos[$i]["objectclass"][1]=="person") {
        		echo "<tr>";
        		echo "<td>".$infos[$i]["cn"][0]."</td><td>".$infos[$i]["dn"]."</td><td>"."tijuana.empresa.com"."</td>";
        		echo "</tr>";
        	}
        }
        echo "</table>";
