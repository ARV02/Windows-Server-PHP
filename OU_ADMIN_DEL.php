<?php include('seguridad.php');
//Lectura del archivo de texto UO.txt
$myfile = "UO.txt";
$value= file($myfile);
$var = rtrim($value[0]);
$var2 = $var;
$uo = explode(",", $var2);
$dominio = explode(",", $var2);
$subdominio = explode(",", $var2);
$dominiohijo = explode(",", $var2);
$conectado=ldap_connect("empresa.com") or die ("no conectado");
ldap_set_option($conectado, LDAP_OPT_PROTOCOL_VERSION,3);
ldap_set_option($conectado, LDAP_OPT_REFERRALS,0);

if($dominiohijo[1] == ""){
	if($conectado){
		$autenticado = @ldap_bind($conectado, "Administrador@empresa.com", "admin-10");
		if($autenticado){
			$info["ou"]= $uo[0];
			$info["objectclass"]="organizationalUnit";
			$info["l"]="location";
			try{
				$ad=@ldap_delete($conectado,"ou=$uo[0],dc=$dominio[2],dc=$subdominio[3]");
				if($ad)
					echo "OU eliminada";
				else
					echo "OU no eliminada";
			}catch(Eception $e){
				echo $e;
			}
		}
	}else{
		echo "Sin conexion";
	}
}else{
	$conectado=ldap_connect("$dominiohijo[1].empresa.com") or die ("no conectado");
	ldap_set_option($conectado, LDAP_OPT_PROTOCOL_VERSION,3);
	ldap_set_option($conectado, LDAP_OPT_REFERRALS,0);
	if($conectado){
		$autenticado = @ldap_bind($conectado, "Administrador@$dominiohijo[1].empresa.com", "admin-10");
		if($autenticado){
			$info["ou"]= $uo[0];
			$info["objectclass"]="organizationalUnit";
			$info["l"]="location";
			try{
				$ad=@ldap_delete($conectado,"ou=$uo[0],dc=$dominiohijo[1],dc=$dominio[2],dc=$subdominio[3]");
				if($ad)
					echo "OU eliminada";
				else
					echo "OU no eliminada";
			}catch(Eception $e){
				echo $e;
			}
		}
	}else{
		echo "No hay conexion";
	}
}
?>