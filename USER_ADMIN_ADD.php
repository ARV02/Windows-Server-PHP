...
if($dominiohijo[4] == ""){
    if($conectado){
        echo "Conectado";
        echo "<br>";
        $autenticado = @ldap_bind($conectado, "Administrador@empresa.com", "RedesPro2020");
        if($autenticado){
            $info["cn"] = $cn[0];
            $info["sn"] = $cn[0];
            ...
            try{
                $ad=@ldap_add($conectado,"cn=$cn[0],ou=$ou[2],dc=$dominio[5],dc=$subdominio[6]",$info);
                $se = ldap_search($conectado, "dc=$dominio[5],dc=$subdominio[6]","cn=$cn[0]");
   ...
   if($conecta){
        echo "Conectado";
        echo "<br>";
        $autenticado = @ldap_bind($conecta, "Administrador@$dominiohijo[4].empresa.com", "RedesPro2020");
        if($autenticado){
            $info["cn"] = $cn[0];
            $info["sn"] = $cn[0];
            $info["ObjectClass"] = "user";
            $info["UserAccountControl"] = "544";
            try{
                $ad=@ldap_add($conecta,"cn=$cn[0],ou=$ou[2],dc=$dominiohijo[4],dc=$dominio[5],dc=$subdominio[6]",$info);
                $se = ldap_search($conecta, "dc=$dominiohijo[4],dc=$dominio[5],dc=$subdominio[6]","cn=$cn[0]");
                $infos = ldap_get_entries($conecta, $se);
                if($ad)
                    echo "Usuario creado";
                else
                    echo "Usuario no ha sido creado";
            }catch(Exception $e){
                echo $e;
            }
        }else{
